        ...
        getData: function (a, c) {
            ...
            switch (a.action) {
                ...
                case 'streamium_get_dynamic_series_content':
                    _url = [streamium_object.ajax_url, a.action, a.post_id, a.postId].filter(function (e) {
                        return e
                    }).join('/') + '.json';
                    break;
                default:
                    _url = [streamium_object.ajax_url, a.action, a.post_id, a.postId, a.query && a.query['name'] ? a.query['name'].toLowerCase() : null].filter(function (e) {
                        return e
                    }).join('/') + '.json';
            }
            b.ajax({
        ...
        getMovieData: function (a, c) {
            ...
            _successFn = function (c) {
                ...
                var f = "." + c.cat,
                    ...
                    k = b(f).width(),
                    l = Math.floor(k / 21 * 8);
                b(f).find("h2.synopis").text(c.title), b(f).find("div.synopis").html(e), b(f).find("a.synopis").attr("href", c.href), b(f).css("background-image", "url(" + (c.tileUrlExpanded || c.tileUrl) + ")"), "" === c.trailer ? b(f).find("a.synopis-video-trailer").hide() : b(f).find("a.synopis-video-trailer").fadeIn().attr("href",  c.trailer);//c.href + "?trailer=true");
                ...
                b("html, body").animate({
                    ...
                    }), b(i).show(), b(i).addClass("tile-white-is-selected"), b('[data-toggle="tooltip"]').tooltip()
                });
                        ...
                        for (var k = 0; k < Object.keys(c).length; k++) {
                            ...
                            if (l.length > 0)
                                for (var m = 0; m < l.length; m++) {
                                    var n = i++;
                                    d.isMobile.any() ? e += '<div class="tile-series"><a class="play-icon-wrap" href="' + l[m].link + "?v=" + n + '"><div class="tile_inner" style="background-image: url(' + l[m].tileUrl + ');"><div class="overlay-gradient"></div><h4><b>S' + l[m].seasons + ":E" + l[m].positions + "</b> " + l[m].title + "</h4></div></a></div>" : e += '<div class="tile-series"><div class="tile_inner lazy" data-src="' + l[m].tileUrl + '"><div class="overlay-gradient"></div><a class="play-icon-wrap" href="' + l[m].link + '"><div class="play-icon-wrap-rel"><div class="play-icon-wrap-rel-ring"></div><span class="play-icon-wrap-rel-play"><i class="fa fa-play fa-1x brand-color" aria-hidden="true"></i></span></div></a><h4><b>S' + l[m].seasons + ":E" + l[m].positions + "</b> " + l[m].title + "</h4></div></div>"
                                }
                        }
                    b(h).fadeIn(), b(f).find("h4.series-watched-episode-title").text("Episodes"), !1 === b(g).hasClass("slick-initialized") && (p.html(e), b(g).slick(d.slickSeries), b(g).on("setPosition", function (a, c, e) {
                ...
                if (a._data.series) {
                    ...
                    if (/^2PACX/.test(_series)) {
                        ...
                        models.getData(_series, function (_e1) {
                            console.log('_e1');
                            console.log(_e1);
//                            _e1 = _updatePostMedia(_e1, _series);
  //                          console.log(_e1);
                            var _ee = _e1.map(function (_e2) {
                                console.log(_e2);
                                return _updatePostMedia(_e2, _series, _e2.id);
                            })
                    ...
                    _.filter(streamium_object._data.category, function (_e) {
                        ...
                        if (_e.data) {
                            _post = _.filter(_e.data, function (_e1) {
                                return _e1.post.ID == e;
                            })[0];
                        } else if (_e.post.ID == e) {
                            _post = _e;
                        }
                        console.log(_post);
                        if (_post) {
...
// 			_obj._data.content = _obj._data.text;
            ...
            _more_content_render = function (b) {
                console.log(b);
                a(".streamium-review-panel-content").html(b.content),
                    a(".streamium-review-panel-header h1").html(b.title),
                    a(".streamium-review-panel").addClass("is-visible"), a("html, body").addClass("overflow-hidden")
            },
...
//		console.log('1.2');
        ...
        }), _home_archive_render = function (//b, d,
                                             e) {
        ...
        && (streamium_object._data.category ? (
            ...
            a.when(
                ...
                a.ajax({
                    ...
                    //url: streamium_object.ajax_url,
                    url: [streamium_object.ajax_url, "home_api_post", streamium_object.query && streamium_object.query['name'] ? streamium_object.query['name'] : null].filter(function (e) {
                        return e
                    }).join('/') + '.json',
                    data: {
            ...
            _tax_render = function (b) {
                ...
                setTimeout(function(){
                    var _e = a('a.synopis-video-next');
                    console.log(_e);

                    var _iar = window.location.hash.split('*');
                    console.log(_iar);
                    var _id = 0;
                    if(_iar.length > 1)
                        _id = +_iar[_iar.length - 1];
                    console.log(_id);
                    var _nid = window.location.hash + '*1';
                    if(_id > 0) {
                        var __id = parseInt(_id) + 1;
                        _nid = window.location.hash.replace('*' + _id, '*' + __id);
                    } 
                      

                    if(_e) {
                        _e.each(function(_i,e){e.href = window.location.href.split('#')[0] +  _nid;});
                    }
                }, 1000);
            },
