TinyCore.Module.define('models_vidtop', [], function(){
    var _cache = lru(100);
    var _ttl = 600000;
    function _copy(aObject) {
	var bObject, v, k;
	bObject = Array.isArray(aObject) ? [] : {};
	for (k in aObject) {
	    v = aObject[k];
	    bObject[k] = (typeof v === "object") ? _copy(v) : v;
	}
	return bObject;
    }
    return {
	...
	getPlayObj: function(_gd_key, _cb){
		...
		that.getData(_gd_key, function(_e2_) {
		    ...
		    // })[0];
		    console.log('e2'); console.log(_e2_);
...
//					   function(_e3) {
	...
	getDataPlayDetail: function(_e2, callback){
		...
		console.log('_e3333333');console.log(_e3_);
...
//		if (_e3.options.youtube){
	...
	_getDataHandler:  function(_res, key, _id, callback){
	    ...
	    var _val;
		if(_id)
		    _val = _tmp.filter(function(_e){ return _e.id == _id})[0];
		else
		    _val = _tmp;
...
//	    if(_cache[key]) return callback(_cache[key]);
	    ...
	    console.log("key1_cached:" + key1);console.log(_cc);
	    if(_cc) {
		...
		that._api(_url, function(_val){
		    console.log("key1:" + key1);console.log(_val);
		    if(vidtop.config && vidtop.config.player && key1 == vidtop.config.player)
